...
<xsl:template match="/pokedex">
    Strong attackers:
    <xsl:apply-templates select="pokemon[number(attack) > 150]" />
    Strong defenders:
    <xsl:apply-templates select="pokemon[number(defense) > 150]"/>
    Fast:
    <xsl:apply-templates select="pokemon[number(speed) > 150]" />
</xsl:template>
