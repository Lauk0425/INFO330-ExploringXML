...
<xsl:apply-templates select="pokemon[type[position()=1]]" />
</xsl:template>

<xsl:template match="pokemon">
    <xsl:value-of select="name" /> (<xsl:value-of select="@pokedexNumber" />): <xsl:value-of select="type" /> | <xsl:value-of select="ability" /> |
</xsl:template>
